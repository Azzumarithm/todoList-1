...
const Drag = () => {
    ...
    function getDragAfterElement(listContainer, y) {
        const draggableElements = [...listContainer.querySelectorAll('.todo-item.draggable:not(.dragging)')]
    ...
    function updateTodoOrder() {
        const orderedTodos = [...document.querySelectorAll('.todo-item')].map((todo, index) => {
          const existingTodo = todos.find(t => t.content === todo.querySelector('.todo-content input').value);
          return {
            ...existingTodo,
            createdAt: new Date().getTime(),
            number: 1
